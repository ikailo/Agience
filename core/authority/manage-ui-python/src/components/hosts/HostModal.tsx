...
export const HostModal: React.FC<HostModalProps> = ({
  ...
  const [formData, setFormData] = useState<HostFormData>({
    name: '',
    address: '',
    operatorId: user?.profile?.sub || '', // Adjust based on your User structure
  });
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.address.trim()) {
      newErrors.address = 'Host address is required';
    } else if (!/^https?:\/\/.+/.test(formData.address)) {
      newErrors.address = 'Host address must be a valid URL starting with http:// or https://';
    }
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-4">
              ...
              {/* Operator ID */}
              <div>
                <label htmlFor="operatorId" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Operator ID
                </label>
                <input
                  type="text"
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Using your account ID by default. Change only if needed.
                </p>
              </div>
            </div>
